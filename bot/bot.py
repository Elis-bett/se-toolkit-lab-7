...
def run_test_mode():
    """Run bot in CLI test mode."""
    parser = argparse.ArgumentParser()
    parser.add_argument("--test", help="Command to test, e.g., /start or /scores lab-04")
...
def run_telegram_mode():
    ...
    try:
        ...
        app.add_handler(CommandHandler("scores", lambda u, c: scores(u.message.text.replace("/scores", "").strip())))
        
        app.run_polling()
    except ImportError:
        print("❌ python-telegram-bot not installed. Run: uv sync")
        sys.exit(1)
